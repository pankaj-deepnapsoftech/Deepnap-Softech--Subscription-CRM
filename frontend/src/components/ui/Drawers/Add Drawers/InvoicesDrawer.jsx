...
const InvoicesDrawer = ({ closeDrawerHandler, getAllInvoices }) => {
  ...
  const addProformaInvoiceHandler = async (e) => {
    ...
    const products = items.map((prod) => {
      return {
        product: prod.item, quantity: parseInt(prod.quantity), price: parseInt(prod.price), total: parseInt(prod.total)
      }
    });
    ...
    try {
      const baseUrl = process.env.REACT_APP_BACKEND_URL;
      const response = await fetch(baseUrl + "invoice/create-invoice", {
        method: "POST",
        headers: {
  ...
  return (
    <div
      className="overflow-auto absolute h-[100vh] w-[90vw] md:w-[550px] bg-white right-0 top-0 z-10 py-3"
      style={{
        boxShadow:
