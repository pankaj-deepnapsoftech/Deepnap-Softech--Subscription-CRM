...
const ExpensesCategoryDrawer = ({closeDrawerHandler, fetchAllExpensesCategory}) => {
  ...
  const addExpensesCategoryHandler = async (e)=>{
    ...
    try{
      ...
      const response = await fetch(baseURL+'expense-category/create-category', {
        method: "POST",
        headers: {
  ...
  return (
    <div className="absolute overflow-auto h-[100vh] w-[90vw] md:w-[450px] bg-white right-0 top-0 z-10 py-3" style={{boxShadow: "rgba(0, 0, 0, 0.08) 0px 6px 16px 0px, rgba(0, 0, 0, 0.12) 0px 3px 6px -4px, rgba(0, 0, 0, 0.05) 0px 9px 28px 8px"}}>
      <h1 className="px-4 flex gap-x-2 items-center text-xl py-3 border-b"><BiX onClick={closeDrawerHandler} size="26px" />Expense Category</h1>

      <div className="mt-8 px-5">
        <h2 className="text-2xl font-semibold py-5 text-center mb-6 border-y bg-[#f9fafc]">Add New Category</h2>

        <form onSubmit={addExpensesCategoryHandler}>
